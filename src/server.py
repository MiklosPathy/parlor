...
MODELS = {
    "4b": ("litert-community/gemma-4-E4B-it-litert-lm", "gemma-4-E4B-it.litertlm"),
    "12b": ("litert-community/gemma-4-12B-it-litert-lm", "gemma-4-12B-it.litertlm"),
}
MODEL_KEY = os.environ.get("MODEL", "4b")
HF_REPO, HF_FILENAME = MODELS.get(MODEL_KEY, MODELS["4b"])
...
def load_models():
    ...
    print(f"Loading Gemma 4 {MODEL_KEY.upper()} from {MODEL_PATH}...")
    engine = litert_lm.Engine(
        MODEL_PATH,
        backend=litert_lm.Backend.GPU,
        vision_backend=(litert_lm.Backend.GPU if MODEL_KEY == "4b" else None),
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        while True:
            ...
            print(f"[in] audio={has_audio} content_parts={len(content)}", flush=True)

            # LLM inference
            t0 = time.time()
            tool_result.clear()
            try:
                print("[llm] send_message start", flush=True)
                response = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: conversation.send_message({"role": "user", "content": content})
                )
                print("[llm] send_message done", flush=True)
